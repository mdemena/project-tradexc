...
class SupportController {
	...
	static async set(_support) {
		const editSupport = await Support.findByIdAndUpdate(_support._id, _support);
		if (editSupport) {
	...
	static async list(_userId) {
		return await Support.find(_userId);
	}
	static async findOne(_filter) {
		return await Support.findOne(_filter);
	}
	static async registerLog(_support, _action) {
		await LogController.register(`${_action} support ${_support._id}`, _support.user);
    }
}
